# ==========================================================
# Candidate Replay Runner - Engine Replay Binding via MT5 Shim
# File: C:\Hybrid_Intelligence_Mentor\candidate_replay_runner_v1_0_6.py
# Version: 1.0.6
# Changelog:
# - v1.0.6:
#   * Bind engine.TradingEngine.generate_signal_package() which has signature ()
#   * Implement MT5Shim and inject into engine module (engine.mt5 = shim)
#   * Shim returns historical bars up to current replay time (no-lookahead)
#   * JSON-safe export (Timestamp/datetime -> ISO)
#   * Engine error log: .state/engine_eval_errors.log
#
# Backtest Evidence:
...
# -----------------------------
class _SymbolInfo:
    def __init__(self, spread: int = 10, digits: int = 2, point: float = 0.01):
        self.spread = spread
        self.digits = digits
        self.point = point
...
class MT5Shim:
    """
    Emulates a minimal subset of MetaTrader5 API used by engine.py
    ...
    def __init__(self, real_mt5, symbol: str, tf_map: Dict[int, "pd.DataFrame"], state_dir: str):
        ...
        # Build reverse timeframe map (const -> name) from real mt5 when possible
        self._tf_name = {}
        try:
            self._tf_name[self._real.TIMEFRAME_M1] = "M1"
            self._tf_name[self._real.TIMEFRAME_M5] = "M5"
            self._tf_name[self._real.TIMEFRAME_M15] = "M15"
            self._tf_name[self._real.TIMEFRAME_M30] = "M30"
            self._tf_name[self._real.TIMEFRAME_H1] = "H1"
            self._tf_name[self._real.TIMEFRAME_H4] = "H4"
            self._tf_name[self._real.TIMEFRAME_D1] = "D1"
        except Exception:
            pass

    def _log(self, tag: str, msg: str) -> None:
        path = os.path.join(self._state_dir, "mt5_shim.log")
        with open(path, "a", encoding="utf-8") as f:
            f.write(f"{tag}: {msg}\n")
    ...
    def symbol_info(self, symbol: str):
        if symbol != self._symbol:
            return None
        # If real mt5 has info, keep it (more realistic)
        try:
            si = self._real.symbol_info(symbol)
            if si is not None:
                return si
        except Exception:
            pass
    ...
    def symbol_info_tick(self, symbol: str):
        # Optional: try real mt5; if unavailable, return None-like
        try:
            return self._real.symbol_info_tick(symbol)
        except Exception:
            return None

    def _df_for_tf(self, timeframe: int) -> Optional["pd.DataFrame"]:
        return self._tf_map.get(timeframe)
    ...
    def _df_to_rates(self, df: "pd.DataFrame"):
        """
        Convert DataFrame to list[dict] shaped like MT5 rates.
        MT5 uses 'time' as seconds since epoch.
        """
        if df is None or len(df) == 0:
            return []
...
# -----------------------------
# Engine Adapter (signature = ())
# -----------------------------
class EngineReplayAdapter:
    def __init__(self, config_path: str, engine_module, engine_obj, shim: MT5Shim, state_dir: str, debug: bool):
    ...
    def evaluate(self) -> Tuple[bool, Dict[str, Any]]:
        """
        Call engine.generate_signal_package() with no args.
        Decide candidate from returned package.
        """
        try:
...
def main() -> None:
    ...
    # Load historical rates from REAL MT5 once
    ...
    # We will shutdown real mt5 after data load to prevent engine using live data
    mt5.shutdown()
    ...
    # Build shim and inject into engine module
    try:
    ...
    # IMPORTANT: swap engine.mt5 to shim
    try:
        engine.mt5 = shim  # type: ignore
    except Exception:
        pass

    # Create engine instance (it will see shim as mt5)
    try:
        eng = TradingEngine(args.config)
    except Exception as e:
        die(f"TradingEngine init failed under shim: {type(e).__name__}: {str(e)}")

    adapter = EngineReplayAdapter(
        config_path=args.config,
        engine_module=engine,
        engine_obj=eng,
        shim=shim,
        state_dir=args.state_dir,
        debug=args.debug_engine,
    )

    print("============================================================")
    print("CANDIDATE REPLAY RUNNER v1.0.6")
    ...
    for i in range(total):
        ...
        # set replay time for shim (no-lookahead)
        shim.current_time = t_ltf

        # Evaluate engine at this replay time (engine reads from shim)
        is_candidate, pkg = adapter.evaluate()
        if isinstance(pkg, dict) and "error" in pkg:
            eval_errors += 1
    ...
    print(f"MT5 shim log: {os.path.join(args.state_dir, 'mt5_shim.log')}")
    print("============================================================")
