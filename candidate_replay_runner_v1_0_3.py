...
# File: C:\Hybrid_Intelligence_Mentor\candidate_replay_runner_v1_0_3.py
# Version: 1.0.3
# Changelog:
# - v1.0.3:
#   * Fix bar sufficiency check: allow Have == Need (use < instead of <=)
#   * Add CLI override: --warmup to support short-range dry runs
#   * Keep v1.0.2 debugging features (force-candidate, progress logs, counters)
#
# Backtest Evidence:
...
def main() -> None:
    ...
    ap.add_argument("--warmup", type=int, default=None, help="Override warmup bars for short-range tests")
    args = ap.parse_args()
    ...
    for i in range(start_idx, total):
        ...
        if args.force_candidate:
            is_candidate = True
            pkg: Dict[str, Any] = {"forced": True, "note": "force_candidate enabled - pipeline test"}
            fallback_hits += 1
        else:
            try:
        ...
        if is_candidate:
            snap = build_snapshot(symbol, cfg, t_ltf, ltf_slice, mtf_slice, htf_slice, pkg, spread_proxy)
            event = {
